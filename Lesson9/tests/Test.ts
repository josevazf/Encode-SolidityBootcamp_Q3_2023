...
describe("Basic tests for understanding ERC20", async () => {
  ...
  it("triggers the Transfer event with the address of the sender when sending transactions", async function () {
	...
	const minTx = await MyERC20Contract.mint(senderAddress, 2);
	await minTx.wait();
	await expect(MyERC20Contract.transfer(receiverAddress, 1)).to.emit(MyERC20Contract, "Transfer").withArgs(senderAddress, receiverAddress, 1);
  });
});