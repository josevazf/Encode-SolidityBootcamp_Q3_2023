...
async function main() {
	...
	if (args[0] == "giveRightToVote") {
		giveRightToVote(args[1]);
	} else if (args[0] == "delegate") {
		delegate(args[1]);
	} else if (args[0] == "vote") {
		vote(Number(args[1])); // CONVERT TO NUMBER!!!
	} else if () {
...
/* 	const proposals = process.argv.slice(2);
	...
	async function vote(proposalNumber: number) {
		...
		console.log(`Vote for proposal ${proposalNumber + 1}:`, `"${decodeBytes32String(votedProposal.name)}"`, "submited");
